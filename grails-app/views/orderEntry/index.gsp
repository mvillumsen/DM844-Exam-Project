		...
		<div id="list-orderEntry" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="dateCreated" title="${message(code: 'orderEntry.dateCreated.label', default: 'Date Created')}" />
					
						<g:sortableColumn property="lastUpdated" title="${message(code: 'orderEntry.lastUpdated.label', default: 'Last Updated')}" />
				...
				<g:each in="${orderEntryInstanceList}" status="i" var="orderEntryInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${orderEntryInstance.id}">${fieldValue(bean: orderEntryInstance, field: "amount")}</g:link></td>
