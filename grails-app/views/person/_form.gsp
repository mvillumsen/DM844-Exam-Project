...
<div class="fieldcontain ${hasErrors(bean: personInstance, field: 'address', 'error')} ">
	...
	<g:select id="address" name="address.id" from="${dk.dm844.webshop.Address.list()}" optionKey="id" value="${personInstance?.address?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: personInstance, field: 'dateHired', 'error')} ">
	...
	<g:datePicker name="dateHired" precision="day"  value="${personInstance?.dateHired}" default="none" noSelection="['': '']" />
...
<li class="add">
<g:link controller="productOrder" action="create" params="['person.id': personInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'productOrder.label', default: 'ProductOrder')])}</g:link>
</li>
</ul>
