...
<div class="fieldcontain ${hasErrors(bean: orderEntryInstance, field: 'order', 'error')} required">
    ...
    <g:select id="order" name="order.id" from="${dk.dm844.webshop.ProductOrder.list()}" optionKey="id" required=""
              value="${orderEntryInstance?.order?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: orderEntryInstance, field: 'product', 'error')} required">
    ...
    <g:select id="product" name="product.id" from="${dk.dm844.webshop.Product.list()}" optionKey="id" required=""
              value="${orderEntryInstance?.product?.id}" class="many-to-one"/>
