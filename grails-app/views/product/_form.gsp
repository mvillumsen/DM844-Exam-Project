...
<div class="fieldcontain ${hasErrors(bean: productInstance, field: 'category', 'error')} required">
    ...
    <g:select id="category" name="category.id" from="${dk.dm844.webshop.Category.list()}" optionKey="id" required=""
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="orderEntry" action="create"
                    params="['product.id': productInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'orderEntry.label', default: 'OrderEntry')])}</g:link>
        </li>
    </ul>
