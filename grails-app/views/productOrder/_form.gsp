...
<div class="fieldcontain ${hasErrors(bean: productOrderInstance, field: 'assignedEmployee', 'error')} ">
    ...
    <g:select id="assignedEmployee" name="assignedEmployee.id" from="${dk.dm844.webshop.Person.list()}" optionKey="id"
              value="${productOrderInstance?.assignedEmployee?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: productOrderInstance, field: 'orderEntries', 'error')} ">
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="orderEntry" action="create"
                    params="['productOrder.id': productOrderInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'orderEntry.label', default: 'OrderEntry')])}</g:link>
        </li>
    </ul>
...
<div class="fieldcontain ${hasErrors(bean: productOrderInstance, field: 'address', 'error')} required">
    ...
    <g:select id="address" name="address.id" from="${dk.dm844.webshop.Address.list()}" optionKey="id" required=""
              value="${productOrderInstance?.alternativeAddress?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: productOrderInstance, field: 'customer', 'error')} required">
    ...
    <g:select id="customer" name="customer.id" from="${dk.dm844.webshop.Person.list()}" optionKey="id" required=""
              value="${productOrderInstance?.customer?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: productOrderInstance, field: 'status', 'error')} required">
    ...
    <g:select name="status" from="${dk.dm844.webshop.ProductOrder$Status?.values()}"
              keys="${dk.dm844.webshop.ProductOrder$Status.values()*.name()}" required=""
              value="${productOrderInstance?.status?.name()}"/>
